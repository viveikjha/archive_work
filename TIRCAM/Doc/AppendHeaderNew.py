...
# Files to be Read
# ------------------------------------------------------------------------------------------------------------------- #
#DIR_DOC = '/home/sapna/ARIES_DataArchival/Data/Dummy/Doc/'
DIR_DOC = '/home/bharti/data_archival_project/Dummy/Doc/'
...
def identify_imagetype(filename):
    ...
    Returns:
        ...
        False    : When the file is a bias or a flat or a dark
    """
    with fits.open(filename, mode='update') as hdulist:
        header = hdulist[0].header

        if header[OBJECT_keyword] in ['BIAS', 'FLAT', 'DARK']:
...
#            header. append(card=(keyword, str(radec[idx])))
    ...
    customSimbad = Simbad()
    result = customSimbad.query_object(header[OBJECT_keyword])
    result['MAIN_ID'][0]
    result['RA'][0]
    result['DEC'][0]
    c=result['RA'][0]
    ...
    f=SkyCoord(c,d, unit=(u.hourangle, u.deg))
#print(f)

#    header['RA'] = c
#    header['DEC'] = d
#    header['RA'] = f.ra.degree
#    header['DEC'] = f.dec.degree

    dict_cod = {'RA': f.ra.degree, 'DEC': f.dec.degree}

    for keyword, value in dict_cod.items():
...
##Even if you want to edit the VOTable fields that are currently returned in the result are set to main_id and coordinates. However you can specify other fields that you also want to be fetched in the result (see saved links).
...
def execute_task(ctext, telescopename):
    ...
    for filename in glob.glob(ctext):
        ...
        identify_imagetype(filename)
        # fix_invalidheaders(filename)

  # Performs Astrometry and updates the Header with Astrometry Keywords for the appropriate files only
    for filename in glob.glob(ctext):
