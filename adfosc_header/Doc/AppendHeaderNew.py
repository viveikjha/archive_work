...
def update_adfosc_header(filename, headerlist):
    '''
    Updates the header of ADFOSC files.
    '''
    
    entries= headerlist[:,1]
    with fits.open(filename, mode='update') as hdulist:
        ...
        if 'DATE-OBS' in header.keys():
            ...
            header.append(card=('FRAME', keywords_df.loc[keyword, 'Value']))
            
        
        for keyword in keywords_df.index:
            if keyword == entries[i]
...
###update 12/05/23: Modify dateobs to be added as UT in place of IST.
